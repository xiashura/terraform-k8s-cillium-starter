...
resource "null_resource" "download-k8s-context" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      rsync -e "ssh -o StrictHostKeyChecking=no" -Wav --progress ${var.ssh-user-master-node}@${var.ssh-host-master-node}:/etc/kubernetes/admin.conf ${var.context-path}/context/${var.cluster-name}
    EOF
  }
